...
const ResultFootball = () => {
  const { dataFootball, loading } = useSelector(createStructuredSelector({
    ...
    <Row>
      <Col span={20} offset={2}>
        ...
        {dataFootball.map((item, index) => (
          <Row key={index} style={{ border: '1px solid #ccc', marginBottom: '20px', padding: '20px' }}>
            <Link to={`/tran-dau/${item.home_team.team_name}-vs-${item.away_team.team_name}/${item.fixture_id}`}>
              <Col span={24}>
                <p>{item.round}</p>
              </Col>
              <Col span={10}>
